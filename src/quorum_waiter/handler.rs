...
/// This struct serially waits for to obtain a quorum of acknowledgements before moving to handling another quorum waiting.
    ...
    MsgId: Send + Sync + 'static + std::fmt::Debug,
{
    ...
    async fn run(&mut self) -> Result<()> {
        while let Some(Message{batch, handlers}) = self.rx.recv().await {
            let mut wait_future: FuturesUnordered<_> = handlers.into_iter()
                .map(|handle| {
                    Self::waiter(handle)
                }).collect();
